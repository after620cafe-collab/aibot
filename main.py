...
EXTENSIONS = [
    "cogs.ai", "cogs.fun", "cogs.moderation", "cogs.automation",
...
@bot.event
async def on_ready():
    logging.info("Rani AI ONLINE: %s (%s) | prefix=%s | guilds=%s", bot.user, bot.user.id, PREFIX, len(bot.guilds))
    try:
        if SYNC_GUILD_ID.isdigit():
            guild = discord.Object(id=int(SYNC_GUILD_ID))
            # Copy the complete local command tree to the user's server for instant slash availability.
        ...
        logging.exception("Slash sync failed. Check applications.commands scope and SYNC_GUILD_ID.")
...
async def load_extensions():
    for ext in EXTENSIONS:
        try:
            ...
            # Keep startup alive so one optional cog cannot kill the whole bot.
