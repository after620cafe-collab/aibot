...
class AI(commands.Cog):
    ...
    @app_commands.describe(language="Target language, e.g. Hindi, English, Gujarati", text="Text to translate")
    async def slash_translate(self, interaction: discord.Interaction, language: str, text: str):
        await interaction.response.defer(thinking=True)
        try:
            result = await ask([
                {"role": "system", "content": "You are a precise translator. Return only the translation, with no quotes or explanation."},
                {"role": "user", "content": f"Translate this into {language}:\n{text}"},
            ])
            await interaction.followup.send(result)
        except Exception as e:
            await interaction.followup.send(f"⚠️ **Translation unavailable**\n{friendly_error(e)}")
...
async def setup(bot):
    ...
    bot.tree.add_command(cog.slash_ai)
